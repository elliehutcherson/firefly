...
//
// Symbols are uppercase ("AAPL", "BRK.B"); callers normalize before calling.
class MarketDataProvider {
 public:
  ...
  virtual absl::StatusOr<Trade> GetLatestTrade(const std::string& symbol) = 0;
  ...
  virtual absl::StatusOr<std::vector<Bar>> GetDailyBars(
      const std::string& symbol, absl::CivilDay start, absl::CivilDay end) = 0;
  ...
  virtual absl::StatusOr<std::vector<Bar>> GetMinuteBars(
      const std::string& symbol, absl::Time start, absl::Time end) = 0;
};
